...
// this is to hide keyboard after touching outside the input field
        ...
        labelInput.addTextChangedListener {
            updateBtn.visibility = View.VISIBLE // we have updatebtn's visibility as gone by defaut, this makes it visible again
            if(it!!.count()>0)
        ...
        updateBtn.setOnClickListener{
            ...
            if (label.isEmpty())
                ...
                val transaction= Transaction(transaction.id,label,amount,description) //transaction.id is used to map the id and update the txn
                update(transaction)
            }
        }
